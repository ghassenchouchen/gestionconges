...
class DesktopAuthManager extends DesktopLoginManager {
  ...
  Future<oauth2.Client> _getOauthClient(Uri redirectUrl) async {
    var grant = oauth2.AuthorizationCodeGrant(env['googleClientId']!,
        Uri.parse(googleAuthApi), Uri.parse(googleTokenApi),
        httpClient: _JsonAcceptingHttpClient(), secret: env['authClientSecret']);
